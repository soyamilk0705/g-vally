...
import emp.service.BoardService;
...
public class BoardController {
	...
	public String updateform(int num, int pg, Model model, HttpServletRequest request, HttpSession session){		// Model: Request랑 비슷, control이 view로 보내줌 -> model and view 패턴
		System.out.println("BoardController :: updateform() :: ");											// data를 담으면 jsp에서 전달되고 휘발됨, request는 response가 꼭 수반되야 하는데 model and view는 request, response랑 상관없음
		System.out.println("num :: " + num);																		// 웹이 아니어도됨, Spring이 자체적인 데이터구조를 가지고 있는것
		...
		//  웹모듈 중에 "?a=b&c=d"을 찾아서 전달할 수 있는지?
		String query = request.getQueryString();
		System.out.println("Request parameter = " + query);
		request.setAttribute("parameter", query);
		
		return result;  //updateform.jsp
	}
